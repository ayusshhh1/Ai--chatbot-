...
function App() {
  ...
  return (
    <div className={`flex flex-col h-screen transition-colors duration-300 ${darkMode ? 'bg-gray-900' : 'bg-gray-50'
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className={`text-base sm:text-lg font-semibold ${darkMode ? 'text-white' : 'text-gray-900'
                  }`}>AI Assistant</h1>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 sm:px-4 py-2 text-sm rounded-lg transition-all duration-200 ${darkMode ? 'text-gray-300 hover:text-white hover:bg-gray-700' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-100'
                  }`}
              >
        ...
        <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center py-12 sm:py-16">
              <div className={`w-16 h-16 sm:w-20 sm:h-20 rounded-2xl flex items-center justify-center mb-6 shadow-xl ${darkMode ? 'bg-gradient-to-br from-blue-600 to-indigo-600' : 'bg-gradient-to-br from-blue-100 to-indigo-100'
                }`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-2xl">
                {SUGGESTED_PROMPTS.map((prompt, index) => (
                  <button
                    ...
                    className={`group flex items-start gap-3 p-4 rounded-xl text-left transition-all duration-200 ${darkMode ? 'bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-blue-500' : 'bg-white hover:bg-gray-50 border border-gray-200 hover:border-blue-400 shadow-sm hover:shadow-md'
                      }`}
                  >
                    <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors ${darkMode ? 'bg-blue-600 group-hover:bg-blue-500' : 'bg-blue-100 group-hover:bg-blue-200'
                      }`}>
              ...
              <div className={`group relative max-w-[85%] sm:max-w-2xl ${message.role === 'user' ? 'ml-4 sm:ml-12' : 'mr-4 sm:mr-12'}`}>
                ...
                <div className={`flex items-center gap-2 mt-1 ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  ...
                  <button
                    ...
                    className={`opacity-0 group-hover:opacity-100 transition-opacity p-1.5 rounded-lg ${darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-100'
                      }`}
                  >
                    {copiedId === index ? (
          ...
          {isLoading && (
            <div className="mb-6 flex justify-start">
              <div className="max-w-2xl mr-4 sm:mr-12">
                <div className={`rounded-2xl px-4 py-3 ${darkMode ? 'bg-gray-800 border border-gray-700 shadow-lg' : 'bg-white border border-gray-200 shadow-md'
                  }`}>
                  <div className="flex items-center gap-3">
      ...
      {/* Input Area */}
      <div className={`border-t transition-colors duration-300 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'
        }`}>
        <div className="max-w-4xl mx-auto px-4 py-4">
          <form onSubmit={handleSendMessage} className="flex gap-2 sm:gap-3">
            <input
              ...
              className={`flex-1 px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:cursor-not-allowed transition-all ${darkMode ? 'bg-gray-700 border border-gray-600 text-white placeholder-gray-400' : 'bg-gray-50 border border-gray-200 text-gray-900 placeholder-gray-500'
                }`}
            />
            <button
              type="submit"
